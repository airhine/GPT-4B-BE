...
**⚠️ 시간 분산 체크리스트:**
✅ 모든 날짜가 서로 다른가?
✅ 시간도 15분, 30분, 45분 등으로 다양한가?  
✅ 정시(00분)를 피하고 있는가?
✅ 주말보다 평일 위주인가?
✅ 시간 순서가 논리적인가? (명함등록 → 미팅 → 메모 순서)

## 중요 생성 규칙
1. **풍부한 데이터 생성**: 위 최소 수량을 반드시 지켜주세요. 실제 비즈니스 관계처럼 오랜 기간 쌓인 데이터를 시뮬레이션합니다.

2. **⚠️ 시간 분산 규칙 (매우 중요!)** - 관계 그래프 분석을 위해 시간이 겹치면 안 됩니다:
   - **전체 기간**: 2024년 1월부터 2025년 1월까지 (13개월)
   - **명함 등록**: 2024년 1월 (랜덤 날짜/시간)
   - **각 데이터는 서로 다른 날짜/시간**: 절대로 같은 시간에 여러 이벤트 생성 금지!
   - **최소 간격**: 
     * 일정(events): 최소 3일 간격으로 분산 (예: 1/5, 1/9, 1/15, 2/3, ...)
     * 메모(memo): 일정 후 1~2일 뒤 작성 (예: 일정이 1/5면 메모는 1/6 또는 1/7)
     * 선물(gifts): 특별한 날 (생일, 기념일, 명절) 또는 미팅 후
     * 채팅(chats): 선물 구매 1~3일 전
   - **시간대 다양성**: 오전(09:00~12:00), 오후(14:00~18:00), 저녁(19:00~21:00) 골고루 분산
   - **요일 다양성**: 월~금 비즈니스 시간 위주, 가끔 토요일 포함
   - **현실적 순서**: 명함등록 → 첫미팅 → 메모 → 선물대화 → 선물구매 → 후속미팅 → ...

**시간 분산 예시 (이처럼 다양하게!):**
- 명함 등록: 2024-01-03T09:15:00.000Z
- 첫 미팅: 2024-01-08T14:30:00.000Z (5일 후)  
- 미팅 메모: 2024-01-09T16:45:00.000Z (미팅 다음날)
- 선물 대화: 2024-02-12T10:20:00.000Z (한 달 후)
- 선물 구매: 2024-02-14T11:00:00.000Z (대화 2일 후)
- 후속 미팅: 2024-03-05T15:15:00.000Z (3주 후)
→ **절대 같은 날짜나 정시(00분) 사용 금지!**
...
\`\`\`json
{
  ...
  "events": [
    {
      "title": "투자 피칭 미팅",
      "startDate": "2024-01-08T14:30:00.000Z",
      "endDate": "2024-01-08T16:00:00.000Z",
      "category": "미팅",
...
**⚠️ 기존 시간과 절대 중복 금지!**
기존 데이터의 모든 날짜/시간을 확인하고, 완전히 다른 날짜/시간만 사용하세요.
예: 기존에 2024-03-15가 있으면 → 새로운 데이터는 2024-03-19 이후 사용

## ⚠️ 시간 규칙 (매우 중요! 중복 절대 금지!)
- **전체 기간**: 2024년 1월부터 2025년 1월까지
